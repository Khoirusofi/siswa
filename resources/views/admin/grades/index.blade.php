    ...
    <div class="py-1">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-end mb-4">
                <form action="{{ route('admin.grades.index') }}" method="GET" id="filter-form"
                                ...
                                @foreach ($enrollments as $i => $enrollment)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-4 py-3 font-medium text-gray-900">
                                            {{ $i + 1 }}
                                            <input type="hidden" name="grades[{{ $i }}][enrollment_id]"
                                                value="{{ $enrollment->id }}">
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-gray-900 whitespace-nowrap">
                                            {{ optional($enrollments->last()?->room?->major)->name ?? '-' }}
                                        </td>
                                        @foreach ($subjects as $subject)
                                            @php
                                                $existingGrade =
                                                    $existingGrades[$enrollment->id . '-' . $subject->id][0]->score ??
                                                    '';
                                            @endphp
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <input type="number" step="0.1" min="0" max="100"
                                                    name="grades[{{ $i }}][subjects][{{ $subject->id }}]"
                                                    value="{{ old("grades.$i.subjects." . $subject->id, $existingGrade) }}"
