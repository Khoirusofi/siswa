        ...
        <div class="flex justify-between items-center">
            <div>
                <nav class="text-sm font-semibold text-gray-600">
                    <span class="text-gray-500">Tingkat Kelas</span>
                </nav>

                <h2 class="text-2xl font-semibold text-gray-800 mt-1">Daftar Tingkat Kelas</h2>
            </div>

            <x-link-route href="{{ route('admin.levels.create') }}" variant="primary">
                Buat
            </x-link-route>
    ...
    <div class="py-1">
        <div class="max-w-7xl mx-auto px-4 sm:px-4 lg:px-4">
            <div class="flex justify-end items-end text-end mb-4">
                <form action="{{ route('admin.levels.index') }}" method="GET" class="flex items-center"
                    id="search-form">
                    <div class="relative">
                        ...
                        @forelse ($levels as $level)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 font-medium text-gray-900">
                                    {{ $levels->firstItem() + $loop->index }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    {{ $level->name ?? '-' }}</td>
                                <td class="px-4 py-3 flex items-center justify-start gap-2">
                                    <x-link-button href="{{ route('admin.levels.edit', $level->id) }}" variant="warning"
                                        icon="pencil-square" title="Edit {{ $level->name ?? '-' }}">
                                        Edit
                                    </x-link-button>

                                    <form action="{{ route('admin.levels.destroy', $level->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus {{ $level->name ?? '-' }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <x-link-button type="submit" variant="danger" icon="trash"
                                            title="Hapus {{ $level->name ?? '-' }}">
                                            Hapus
                                        </x-link-button>
                                    </form>
                                </td>
