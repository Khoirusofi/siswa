    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <h2 class="text-2xl font-semibold text-gray-800 mt-1">Daftar Tahun Ajaran</h2>
            </div>

            <x-link-route href="{{ route('admin.years.create') }}" variant="primary">
                Buat
            </x-link-route>
    ...
    <div class="py-1">
        <div class="max-w-7xl mx-auto px-4 sm:px-4 lg:px-4">
            <div class="flex justify-end items-end text-end mb-4">
                <form action="{{ route('admin.years.index') }}" method="GET" class="flex items-center"
                    id="search-form">
                    <div class="relative">
                ...
                <script>
                    function handleSearch() {
                        ...
                        if (input.value.trim() === '') {
                            window.location.href = "{{ route('admin.years.index') }}";
                        } else {
                            form.submit();
                        }
                    }
                        ...
                        @forelse ($years as $year)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 font-medium text-gray-900">
                                    {{ $years->firstItem() + $loop->index }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    {{ $year->name ?? '-' }}</td>
                                <td class="px-4 py-3 flex items-center justify-start gap-2">
                                    <x-link-button href="{{ route('admin.years.edit', $year->id) }}" variant="warning"
                                        icon="pencil-square" title="Edit {{ $year->name ?? '-' }}">
                                        Edit
                                    </x-link-button>

                                    <form action="{{ route('admin.years.destroy', $year->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus {{ $year->name ?? '-' }}?')">
                                        @csrf
                                        @method('DELETE')
