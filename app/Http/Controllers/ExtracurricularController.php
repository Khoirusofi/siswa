...
class ExtracurricularController extends Controller
{
    public function index(Request $request)
    {
        ...
        $selectedYear = null;
        $enrollments = collect();
        $existingExtracurricularData = collect();
        ...
        if ($selectedRoomSlug && $selectedYearSlug) {
            ...
            if ($selectedRoom && $selectedYear) {
                ...
                $existingExtracurricularData = Component::whereIn('enrollment_id', $enrollments->pluck('id'))
                    ->get()
                    ->pluck('extracurricular', 'enrollment_id');
            }
        }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'components.*.extracurricular' => 'nullable|string',
        ]);
        ...
        try {
            foreach ($request->components as $componentData) {
                ...
                Component::updateOrCreate(
                    ...
                    ['extracurricular' => $componentData['extracurricular'] ?? null]
                );
            }
            ...
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menyimpan nilai eskul.');
        }
    }
}
