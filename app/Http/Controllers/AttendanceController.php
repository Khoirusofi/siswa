...
class AttendanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $selectedYear = null;
        $enrollments = collect();
        $existingAttendanceData = collect();
        ...
        if ($selectedRoomSlug && $selectedYearSlug) {
            ...
            if ($selectedRoom && $selectedYear) {
                ...
                $existingAttendanceData = Component::whereIn('enrollment_id', $enrollments->pluck('id'))
                    ->get()
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'components.*.attendance' => 'nullable|string',
        ]);
        ...
        try {
            foreach ($request->components as $componentData) {
                ...
                Component::updateOrCreate(
                    ...
                    ['attendance' => $componentData['attendance'] ?? null]
                );
            }

            DB::commit();
            return redirect()->back()->with('success', 'Nilai absen berhasil disimpan.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menyimpan nilai absen.');
        }
    }
}
